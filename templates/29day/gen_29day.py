...
try:
    ...
    # 兜底：仅作者本机隔离 Python 的 site-packages（其他环境请用 pip 安装 pillow / python-pptx）
...
from trip_data import S, get_img, get_spot, get_map, TRIP_TITLE, DECK_DIR, GALLERY, IMG_DIR, real_photo
...
BK = RGBColor(0, 0, 0)
ACCENT = RGBColor(255, 255, 255)  # 原橘色(232,109,78)已改为纯白，契合统一视觉
...
def _cover_crop(src, tw, th):
    """把照片 cover 裁切到目标像素尺寸，返回 BytesIO（JPEG）供 add_picture 使用。"""
...
def build_gallery(data):
    """景点图鉴：43 处精选景点网格（4×3/页），按到达日排序，每张含名称 + Day 标签。"""
    items = sorted(data['items'], key=lambda x: x[2])  # (photo, name, day29)
    cols, rows = 4, 3
    per = cols * rows
    ...
    for pi, pg in enumerate(pages, 1):
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        add_bg(slide, os.path.join(IMG_DIR, 'landscape-01.jpg'), 0.8)
        tb(slide, Inches(0.6), Inches(0.32), Inches(12), Inches(0.6),
           [(f'景点图鉴 · 成都往返全程（{(pi - 1) * per + 1}–{min(pi * per, len(items))}/{len(items)}）', 30, W, True)])
        tb(slide, Inches(0.6), Inches(0.95), Inches(12), Inches(0.4),
           [('43 处必看景点 · G318 去程 AI 写实 + 西藏南线真实实拍 · 按到达日排序', 16, G, False)])
        for idx, (photo, name, day29) in enumerate(pg):
            r = idx // cols
            c = idx % cols
            cx = gx + c * (gw + gap_x)
            cy = gy + r * (gh + gap_y)
            px = int(gw * 150)
            py = int(gh * 150)
            buf = _cover_crop(photo, px, py)
            slide.shapes.add_picture(buf, Inches(cx), Inches(cy), Inches(gw), Inches(gh))
            bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE,
                                         Inches(cx), Inches(cy + gh - 0.42),
                                         Inches(gw), Inches(0.42))
            bar.fill.solid()
            bar.fill.fore_color.rgb = BK
...
# 主序列：在「总览 stats」之后插入景点图鉴，其余按 S 顺序
seq = []
_GALLERY_ITEMS = [(photo or real_photo(n) or get_img(1), n, day) for (n, day, photo) in GALLERY]
for e in S:
    seq.append(e)
    if e[0] == 'stats':
        seq.append(('gallery', {'items': _GALLERY_ITEMS}))
for stype, data in seq:
    builders[stype](data)

output_path = os.path.join(DECK_DIR, f'{TRIP_TITLE}-v4.pptx')
try:
    prs.save(output_path)
except PermissionError:
    output_path = os.path.join(DECK_DIR, f'{TRIP_TITLE}-v4-alt.pptx')
    prs.save(output_path)
